...
import { Loader2, Sparkles, CalendarDays, ChefHat } from "lucide-react";
...
export default function CalendarPage() {
  ...
  const handlePlanUpdated = useCallback(() => {
    // Plan updates via Firestore onSnapshot automatically
  }, []);
      ...
      <div className="max-w-7xl mx-auto px-5 lg:px-8 py-6 pb-28 lg:pb-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            <CalendarDays size={22} className="text-[var(--accent)] hidden sm:block" />
            <h1 className="text-xl font-bold text-[var(--foreground)]">Planificador Semanal</h1>
          </div>
          <WeekSelector
            weekId={weekId}
        ...
        {/* Controls */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 bg-[var(--card)] border border-[var(--border)] rounded-xl p-4">
          ...
          <div className="flex items-center gap-2">
            {hasPlan && isCurrentOrFuture && (
              <button
                ...
                disabled={generating}
                className="px-4 py-2.5 border border-[var(--border)] rounded-lg text-sm font-medium text-[var(--muted)] hover:text-[var(--foreground)] hover:border-[var(--border-light)] transition-colors disabled:opacity-50"
              >
                Regenerar
              </button>
            )}
            ...
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-[var(--accent)] rounded-lg text-sm font-bold text-black hover:bg-[var(--accent-hover)] transition-colors disabled:opacity-40"
            >
              {generating ? (
                <>
        ...
        {planLoading ? (
          ...
          <div className="text-center py-20 border-2 border-dashed border-[var(--border)] rounded-2xl">
            ...
            <p className="text-[var(--muted-dark)] text-sm mb-6 max-w-xs mx-auto">
              Genera tu plan semanal con recetas altas en proteina y bajas en carbohidratos
            </p>
            {isCurrentOrFuture && (
              <button
                onClick={() => handleGenerate(false)}
